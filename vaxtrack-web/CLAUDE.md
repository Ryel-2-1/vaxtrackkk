...
**Route + ETA foundation (OpenRouteService) — IMPLEMENTED on branch `feature/route-eta-foundation`, staging-verified (missing-key state) 2026-07-27:**
...
- **Verified on STAGING 2026-07-27** (no ORS key in `.env.staging`): dispatcher login → Geofence, selected order rendered the Leaflet map + rider/clinic markers + geofence circle + "Inside geofence — 145 m" (existing behavior intact) **and** the new "Route & ETA" section showing the safe "API key not configured" message with no button; no console errors; `build:staging` + `test:rules` (24/24) pass. **Route generation itself (polyline/distance/ETA) is UNVERIFIED pending a real ORS key in `.env.staging`** — a follow-up "verify with key" step. **Not deployed, not merged.**
