...
- **Verified live 2026-07-22 (emulator rider.qa2 + Admin web):** toggle + URL field render; submitting a public HTTPS PNG on `VT-ORD-1783611813231` wrote `proofOfDeliveryUrl` with **no StorageException**; Admin Deliveries drawer switched from "No proof uploaded yet." to the populated state with the correct `<img src>` + "Open image" link; empty state still correct on a proofless order (`VT-ORD-1784056096300`).
- ⚠️ **NOT visually confirmed:** the image *pixels* rendering + "Open image" navigation. The in-app test browser sandbox **blocks all external image hosts** (wikimedia / google / picsum all failed identically), so the preview can't paint here. This is an environment limit, not a code fault — the `<img>` src is correctly wired and will render in a normal browser. **Needs a real-browser spot check to close APD-003/004.**
- This is a **temporary demo fallback, not the production upload path.** Once Storage is provisioned, the camera path is the real one and this toggle can be removed.
