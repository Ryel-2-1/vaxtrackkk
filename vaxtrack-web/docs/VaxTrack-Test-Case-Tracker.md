...
| RTE-005 | Functional Suitability | Dispatcher | Geofence / routeService | Generate route draws polyline + distance + ETA | Order with rider lastLocation + clinicLat/Lng, **real ORS key configured** | Click "Generate route & ETA" | ORS route drawn (green polyline), distance + est. duration + ETA shown, fields saved to order | — | Pending Manual Test | Needs a real `VITE_OPENROUTESERVICE_API_KEY` in `.env.staging` (follow-up verify step); code path built + build/rules pass |
| RTE-006 | Security | — | vite.config.js CSP | CSP allows ORS fetch | Feature calls `api.openrouteservice.org` | Inspect served `connect-src` | `https://api.openrouteservice.org` present in dev + preview CSP | Added to both `devCsp` and `previewCsp`; build passes | Passed | Full network confirmation folded into RTE-005 (needs key) |
