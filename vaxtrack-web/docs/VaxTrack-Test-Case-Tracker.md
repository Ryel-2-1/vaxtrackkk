...
| DCL-004 | Functional Suitability | Dispatcher | CargoLoading | Finalized orders reflect across roles | Finalize done | Check Shipments/Admin/SR/Rider | in_transit everywhere | Finalize of VT-ORD-1784199396106 → **In Transit** on Dispatcher Shipments, Admin Deliveries, and Sales Rep Order Tracking ("Out for Delivery" 75%, the SR label for in_transit); rider fields (QA / 09179876543) preserved; no console errors on any page | Passed | Dispatcher-side Passed 2026-07-17; Admin/SR-side verified live 2026-07-22 |
...
| DCL-006 | Functional Suitability | Dispatcher | CargoLoading | Checkbox promotes assigned → loading | Assigned order in a rider group | Tick the loaded checkbox | `isLoaded: true` + `loadedAt/ByUid/ByEmail`; status `assigned` → `loading` + status audit fields; order stays in the group | Ticking VT-ORD-1784199396106 → 0/1→1/1 loaded (100%), "Loaded" badge, status Assigned→Loading, card "Ready", order stayed visible, Finalize enabled | Passed | Verified live 2026-07-22 (Dispatcher) |
| DCL-007 | Functional Suitability | Dispatcher | CargoLoading | Unchecking does not regress status | Order already `loading` + loaded | Untick the checkbox | `isLoaded: false`, loaded audit cleared, **status stays `loading`** | Unticking → 1/1→0/1 loaded (0%), "Confirm" returns, Finalize re-disabled, **status stayed Loading (no regress to Assigned)** | Passed | Verified live 2026-07-22 (Dispatcher) |
| DSH-010 | Functional Suitability | Dispatcher | Shipments | Competing dispatch actions removed | Orders in assigned / loading | View row actions | No "Start loading" on assigned, no "Dispatch" on loading; only Delay + Cancel | Assigned VT-ORD-1784199396106 → only Delay + Cancel (no "Start loading"); after promotion to Loading → only Delay + Cancel (no "Dispatch") | Passed | Verified live 2026-07-22 (Dispatcher). Cargo Loading is the canonical dispatch path |
| DSH-011 | Functional Suitability | Dispatcher | Shipments | Exception actions still available | Orders in in_transit / delayed | View row actions | "Mark delivered (override)", Delay, Cancel, Resume transit all present and working | in_transit orders show "Mark delivered (override)" + Delay + Cancel; delayed order shows "Resume transit" + Cancel | Passed | Verified live 2026-07-22 (Dispatcher). Mark delivered is a dispatcher override; Rider app is the normal path |
...
| DLM-003 | Functional Suitability | Dispatcher | Geofence | Map/geofence/route deviation | — | — | Deferred until Rider lastLocation writes | Superseded 2026-07-23: rider-position map now implemented (DLM-004/005); geofence + route deviation still deferred (clinic coordinates) | Not Applicable | Info banner updated to "position only" wording |
| DLM-004 | Functional Suitability | Dispatcher | Geofence | Live map renders rider marker from orders.lastLocation | Selected shipment has lastLocation | Select in_transit order with location | Leaflet map card renders, rider marker at the order's coordinates, existing coords text + stale note kept | Leaflet initialized (zoom controls + OSM attribution); marker centered on [14.553, 121.003] (offset −1,0 px from map center after setView; tile-URL math independently confirms 121.0°E/14.56°N); coords text + stale note retained; no console errors | Passed | Verified live 2026-07-23 (Dispatcher). Leaflet + OpenStreetMap, **no API key**. Fixed two legacy fake-map CSS collisions (`.geo3-map` 520px gradient, `.geo3-rider-marker` absolute left/top) by using `geo3-live-*` class names |
| DLM-005 | Functional Suitability | Dispatcher | Geofence | OSM tile imagery paints | Map rendered | View map | Street-map tiles visible | **Tiles paint in real Chrome.** Root cause of the earlier gray tiles was the app's own dev/preview **CSP `img-src`** omitting the tile host — NOT the sandbox and NOT a Leaflet bug; fixed by adding `https://tile.openstreetmap.org` to `img-src` in `vite.config.js` (both dev + preview). Confirmed by user in own Chrome 2026-07-23 | Passed | The sandboxed test browser still can't paint external tiles (blocks all external hosts), so this was verified in the user's real Chrome |
| DLM-006 | Functional Suitability | Dispatcher | Geofence | Empty state unaffected by map | Selected shipment has no lastLocation | Select order without location | No map card; existing "No live location yet" fallback | Selecting VT-ORD-1783769964131 → no map card, "No live location yet" + waiting text shown | Passed | Verified live 2026-07-23 |
| DLM-007 | Usability | Dispatcher | Geofence | Map layout is bigger + responsive | Map rendered | Resize desktop / tablet / mobile | Height ~480/390/320px across breakpoints, full card width, no horizontal overflow, marker stays centered | Desktop 1280 → 480px; tablet 768 → 390px; mobile 375 → 320px; no horizontal overflow at any size; marker centered (offset ≤1px) after each resize via ResizeObserver→invalidateSize; empty state + coords/stale note retained; no console errors | Passed | Verified live 2026-07-23. Width is the card's full width (315px in the detail column at desktop; full width when the grid collapses to 1 column). Meridian card styling kept |
| GEO-001 | Functional Suitability | Admin | Clinics | Optional clinic coordinates | Register clinic modal open | Enter latitude + longitude, save | `latitude`/`longitude` stored as numbers; existing coord-less clinics unaffected | "Geofence Test Clinic" saved lat 14.5995 / lng 120.9842 (type number); 3 seed clinics without coords still display | Passed | Staging-verified 2026-07-26 (`vaxtrack-staging`). Manual entry, no geocoding API |
| GEO-002 | Functional Suitability | Sales Rep | PlaceOrder | Order carries clinic coordinates | Order from a clinic that has coords | Place order | Order stores `clinicLat`/`clinicLng` (numbers); coord-less clinic omits them | VT-ORD-1785057285884 stored clinicLat 14.5995 / clinicLng 120.9842 | Passed | Staging-verified 2026-07-26 |
| GEO-003 | Functional Suitability | Dispatcher | Geofence | Clinic marker + geofence circle | in_transit order with lastLocation + clinicLat/Lng | Select order | Rider marker + amber clinic marker + 300 m circle render | Leaflet map with rider dot + clinic dot + circle overlay all present | Passed | Staging-verified 2026-07-26 |
| GEO-004 | Functional Suitability | Dispatcher | Geofence | Distance + inside/outside status | Order with rider + clinic coords | View map note | Haversine distance + "Inside/Outside geofence" text | Rider 145 m from clinic → "Inside geofence — 145 m (radius 300 m)"; after moving rider to ~400 m → "Outside geofence — 400 m" | Passed | Staging-verified 2026-07-26. Both boundary cases |
| GEO-005 | Reliability | Dispatcher | Geofence | Order without clinic coords → safe fallback | Order has lastLocation, no clinicLat/Lng | Select order | Rider-only map; no clinic marker/circle; "add clinic coordinates" note | After removing the order's coords: rider marker present, clinic marker + circle absent, fallback note shown | Passed | Staging-verified 2026-07-26. Backward-compatible |
| GEO-006 | Security | — | Firestore rules | New order fields need no rule change | Feature adds clinicLat/clinicLng | Run rules tests | 22/22 still pass; no allowlist change | Order create rule has no field allowlist; clinics admin-write; update allowlists untouched → rules tests 22/22 | Passed | No rule/test change required |
| RTE-001 | Reliability | Dispatcher | Geofence / routeService | Missing ORS key → safe state | No `VITE_OPENROUTESERVICE_API_KEY` | Open Geofence, select order with lastLocation | "Route ETA unavailable: API key not configured." shown; no "Generate route" button; no error thrown | Route & ETA section shows the exact message, no button; page renders normally | Passed | Staging-verified 2026-07-27 (`feature/route-eta-foundation`, no key in `.env.staging`) |
| RTE-002 | Compatibility | Dispatcher | Geofence | Existing geofence intact after feature added | Order with rider + clinic coords | Select order | Rider marker + clinic marker + 300 m circle + "Inside/Outside" distance still work | Leaflet map (8 tiles), green rider dot + amber clinic dot + circle, "Inside geofence — 145 m (radius 300 m)"; no console errors | Passed | Staging-verified 2026-07-27. Route section added below, geofence unchanged |
| RTE-003 | Security | Dispatcher | Firestore rules | Dispatcher may write route fields | Rules deployed | Update order with route* fields | Allowed (in `dispatcherOrderFields`) | Emulator test **P13** passes; **PRODUCTION-verified** — faithful dispatcher `:commit` route write to `vaxtrack-bef1b` returned HTTP 200 (correct doc ID) | Passed | `test:rules` 24/24; production rules deployed `--project default` 2026-07-27 + verified live |
| RTE-007 | Functional Suitability | Dispatcher | Geofence (production) | Route write + display + persistence on production | Route-field rules deployed to `vaxtrack-bef1b`; order with `lastLocation` | Write route via dispatcher; reload Geofence | Route saved, polyline drawn, Distance/Duration/ETA shown, persists after reload | On prod order VT-ORD-1784199396106: dispatcher write HTTP 200; after full reload the green polyline + Distance 218 m / Est. duration 31 s / ETA 3:19 PM displayed; no console errors; synthetic test route then cleaned up | Passed | Production-verified 2026-07-27. Used a real ORS route to a synthetic nearby point (no prod clinic coords); write done via faithful REST `:commit` since the UI Generate button is disabled without clinic coords |
| RTE-008 | Reliability | Dispatcher | Geofence (production) | No-coords fallback + dormant-until-coords on production | Prod order has `lastLocation`, no `clinicLat`/`clinicLng` | Open prod Geofence, select order | Disabled "Generate route & ETA" button + "Add clinic coordinates" note; no missing-key message (key configured) | Verified live on production; **production route generation is dormant** until a prod clinic gets `latitude`/`longitude` and an order is placed from it | Passed | No production clinic/order currently has coordinates |
| RTE-004 | Security | Rider | Firestore rules | Rider may NOT write route fields | Rider on own assigned order | Update order with routePolyline | Denied (not in `riderOrderFields`) | Emulator test **N11** passes | Passed | Route is dispatcher-only |
| RTE-005 | Functional Suitability | Dispatcher | Geofence / routeService | Generate route draws polyline + distance + ETA | Order with rider lastLocation + clinicLat/Lng, **real ORS key configured** | Click "Generate route & ETA" | ORS route drawn (green polyline), distance + est. duration + ETA shown, fields saved to order | Staging (VT-ORD-1785072279117): save succeeded (no permission error); green road-route polyline drawn rider→clinic; **Distance 163 m / Est. duration 22 s / ETA 1:51 PM**; fields **persisted across full page reload**; rider+clinic markers + geofence circle intact; no console errors | Passed | Staging-verified 2026-07-27 with real ORS key + route-field rules deployed to `vaxtrack-staging`. Required deploying the updated `dispatcherOrderFields()` allowlist to staging first (pre-deploy write was `permission-denied` — a rules-deploy gap, not a code bug) |
| RTE-006 | Security | — | vite.config.js CSP | CSP allows ORS fetch | Feature calls `api.openrouteservice.org` | Inspect served `connect-src` | `https://api.openrouteservice.org` present in dev + preview CSP | Added to both `devCsp` and `previewCsp`; live ORS POST from the app succeeded on staging (no CSP block) | Passed | Confirmed end-to-end 2026-07-27 (ORS request succeeded in-browser) |
...
| APD-003 | Functional Suitability | Admin | Deliveries | Image preview renders when proof exists | Order **with** proofOfDeliveryUrl | Open drawer | Thumbnail preview loads | Proof image preview paints/loads correctly in normal Chrome on VT-ORD-1783611813231. (The earlier non-paint was only the sandboxed test browser blocking external image hosts, not a code fault) | Passed | Verified in user's own Chrome 2026-07-22 |
| APD-004 | Functional Suitability | Admin | Deliveries | "Open image" opens full image | Order with proofOfDeliveryUrl | Click Open image | Opens in new tab | "Open image" opens the full image in normal Chrome | Passed | Verified in user's own Chrome 2026-07-22 |
...
## 33. Flutter Rider — Google Maps Navigation (branch `feature/rider-google-maps-navigation`)

| Test Case ID | ISO 25010 Category | Role | Module | Test Scenario | Preconditions | Test Steps | Expected Result | Actual Result | Status | Remarks |
|---|---|---|---|---|---|---|---|---|---|---|
| RGM-001 | Maintainability | Rider | Delivery model | Parse dispatcher route fields | Order has clinicLat/Lng + route* | Load assigned order | `clinicLat/clinicLng/routePolyline/routeDistanceMeters/routeDurationSeconds/routeEtaText/routeGeneratedAt` parsed; `hasClinicCoords`/`hasRoute` getters | Static: model + `_toDouble/_toInt` parse added; `flutter analyze` clean | Passed | Additive; existing fields untouched |
| RGM-002 | Maintainability | Rider | Build | flutter_map (OSM) builds on Android, no key/billing | — | `flutter pub get` + `flutter build apk --debug` | Debug APK builds with original Android config | pub get OK (google_maps_flutter fully removed; flutter_map + latlong2 added); APK built (`app-debug.apk`) with original Kotlin 2.1.0 (no key, no Kotlin bump) | Passed | 2026-07-27. Switched from google_maps_flutter to free OSM/flutter_map |
| RGM-003 | Maintainability | Rider | Static | Analyzer clean | — | `flutter analyze` | No issues | No issues found | Passed | 2026-07-27 |
| RGM-004 | Functional Suitability | Rider | DeliveryDetail / DeliveryMap | OSM map renders rider + clinic markers + polyline | Device + order with clinic coords + routePolyline (NO key needed) | Open in_transit delivery | flutter_map (OSM tiles) with rider dot, clinic dot, decoded route line, OSM attribution | — | Pending Manual Test | Needs an order carrying both coords + route (none on prod/staging yet); OSM tiles need no key |
| RGM-005 | Functional Suitability | Rider | DeliveryDetail | ETA + distance card | Order has route fields | Open delivery | Distance / Duration / ETA from routeDistanceMeters/DurationSeconds/EtaText (live ETA from generatedAt+duration) | — | Pending Manual Test | Formatters unit-consistent with web |
| RGM-006 | Functional Suitability | Rider | DeliveryDetail | Open in Google Maps (turn-by-turn) | clinic coords or address | Tap "Open in Google Maps" | External Google Maps opens `dir/?destination=lat,lng` (or address search fallback) | — | Pending Manual Test | url_launcher, no API key |
| RGM-007 | Reliability | Rider | DeliveryDetail / DeliveryMap | Safe fallbacks | Missing coords / route / location | Open various orders | No coords→rider-only or text card; no route→markers only; no location→center clinic; no points→Manila default | — | Pending Manual Test | All branches coded + build-verified; runtime unverified. No key needed (OSM) |
| RGM-008 | Reliability | Rider | DeliveryDetail | No regression to actions/tracking/proof | — | Open delivery, use Mark Delivered / Report Delay; check tracking + proof | Status flow, foreground tracking, proof fallback all still work | — | Pending Manual Test | Those code paths were not changed |
| RGM-009 | Security | Rider | Firestore rules | Rider only reads route fields | Rider on own assigned order | Load order | No write of route fields; no rules change; rules 24/24 | Static: DeliveryService writes only status/location/proof; `riderOrderFields()` untouched | Passed | No rules/index change |
